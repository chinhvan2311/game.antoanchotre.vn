...
const STORAGE_KEYS = { kids: 'hmc_kids', currentKidId: 'hmc_currentKidId', scores: 'hmc_scores' };
...
function getKids(){ state.kids = loadJSON(STORAGE_KEYS.kids, []); return state.kids; }
function saveKids(){ saveJSON(STORAGE_KEYS.kids, state.kids); }
function getScores(){ state.scores = loadJSON(STORAGE_KEYS.scores, []); return state.scores; }
...
function isNameTaken(key){ return state.kids.some(k => k.nameKey === key); }

function suggestNames(base){
  const suffix = ['2','3','5','7','8','9','⭐','🌈','🐱','🚀','Xanh','Đỏ','Vàng','Mây','Sao'];
  const out = [];
  base = base.trim();
...
function createKid(name){
  name = name.trim();
  if(name.length < 2) return { ok:false, err:'Tên hơi ngắn 😅 (>= 2 ký tự)' };
  if(name.length > 20) return { ok:false, err:'Tên hơi dài 😅 (<= 20 ký tự)' };
  const key = normalizeName(name);
  if(isNameTaken(key)) return { ok:false, err:`Tên “${name}” đã có bạn dùng rồi. Con chọn tên khác nhé!`, suggestions: suggestNames(name) };
  ...
  const kid = {
    ...
    badges: [],
    stats: { totalScore: 0, totalStars: 0, correct: 0, wrong: 0, bySubject: { tapdoc: 0, toan: 0, english: 0 } }
  };
...
function renderKidsModal(){
  ...
  state.kids.forEach(k => {
    ...
    btn.onclick = () => { setCurrentKid(k); closeModal('modalKids'); };
...
function leaderboard(range='all'){
  ...
  const map = new Map();
  filtered.forEach(s => map.set(s.kidId, (map.get(s.kidId)||0) + (s.score||0)));
  return Array.from(map.entries()).map(([kidId, score]) => {
    const k = state.kids.find(x => x.id === kidId);
    return { kidId, name: k?.name || 'Unknown', emoji: k?.emoji || '🧒', score };
...
function startSession(subject, mode){
  ...
  state.session = { subject, mode, questions: pickQuestions(subject, mode), idx: 0, wrongInQuestion: 0, sessionScore: 0, sessionStars: 0 };
  showView('game');
  renderQuestion();
}

function scoreForAttempt(a){ if(a<=1) return 10; if(a===2) return 6; return 3; }
...
function handleCorrect(){
  ...
  addScoreEvent({ kidId: k.id, score: delta, stars: star, subject: s.subject, difficulty: DIFF[s.mode] });
  saveKids();
  renderTopBar();
...
function renderQuestion(){
  ...
  $('#gameMeta').textContent = `${SUBJECT_LABEL[s.subject]} • ${s.mode==='easy'?'😊 Dễ':s.mode==='medium'?'😎 Vừa':'🤯 Khó'} • 🎲 Random`;
  $('#gamePrompt').textContent = q.prompt;
  setFeedback('');

  $('#btnSpeak').onclick = () => speak(q.tts?.text || q.prompt, q.tts?.lang || 'vi-VN');
  if(k?.settings?.autoSpeak) setTimeout(() => speak(q.tts?.text || q.prompt, q.tts?.lang || 'vi-VN'), 200);
...
function renderGhepAm(q){
  ...
  wrap.querySelector('#btnCheck').onclick = () => { chosen.join('') === q.answer ? handleCorrect() : handleWrong(); };
...
function renderSapXep(q){
  ...
  [...q.pieces].sort(() => Math.random()-0.5).forEach(w => {
    const b = document.createElement('button');
    b.className = 'piece';
    b.textContent = w;
    b.onclick = () => { chosen.push(w); sentenceEl.textContent = chosen.join(' '); };
    wordsBox.appendChild(b);
  });

  wrap.querySelector('#btnUndo').onclick = () => { chosen.pop(); sentenceEl.textContent = chosen.join(' '); };
  wrap.querySelector('#btnCheck').onclick = () => { chosen.join(' ').trim() === q.answer ? handleCorrect() : handleWrong(); };
...
function renderDienChu(q){
  ...
  q.choices.forEach(ch => {
    ...
    btn.className = 'piece text-center';
    btn.textContent = ch;
    btn.onclick = () => { q.template.replace('_', ch) === q.answer ? handleCorrect() : handleWrong(); };
...
function renderDemEmoji(q){
  const wrap = document.createElement('div');
  const line = Array.from({length: q.count}).map(() => q.emoji).join(' ');
  wrap.innerHTML = `
    <div class="text-center text-3xl">🔢</div>
  ...
  q.choices.forEach(ch => {
    ...
    btn.className = 'piece text-center';
    btn.textContent = ch;
    btn.onclick = () => { String(ch) === String(q.answer) ? handleCorrect() : handleWrong(); };
    choices.appendChild(btn);
  });
  wrap.appendChild(choices);
  return wrap;
...
function wireEvents(){
  ...
  $('#btnOpenBackup').onclick = () => { $('#backupMsg').textContent = ''; openModal('modalBackup'); };
  ...
  $('#kidNameInput').addEventListener('keydown', (e) => { if(e.key==='Enter') $('#btnCreateKid').click(); });
  ...
  // subjects
  $$('.subject').forEach(btn => {
    btn.onclick = () => { state.lastSubject = btn.dataset.subject; startSession(btn.dataset.subject, 'easy'); };
  });

  // modes use last chosen subject
  $$('[data-mode]').forEach(btn => {
    btn.onclick = () => startSession(state.lastSubject || 'tapdoc', btn.dataset.mode);
  });

  // leaderboard
  $$('[data-lb]').forEach(btn => btn.onclick = () => renderLeaderboard(btn.dataset.lb));
